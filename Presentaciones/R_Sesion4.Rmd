...
abline(v=0) # Agrega linea vértical en el origen
```

Para nuestros ejercicios creamos una matriz artificial de datos

```{r}
# Creammos una matriz artificial de 10x10
X <- rnorm(100)
dim(X) <- c(10,10) 
X
```
...
Estas funciones pueden funcionar a veces como alternativas a los loops. La función `apply` permite aplicar otras funciones sobre los renglones o las columnas de una matriz.
...
Ejemplo: supongamos que cada uno de los renglones corresponde a datos de cuatro países. Aquí llamamos a `X$V1` con signo de pesos porque `X` ya es un data frame y su primera columna es la variables `V1`. Aunque también podemos seguirlo llamando como `X[,1]`.
...
tapply(X$V2, list(paises, sexo), sum) # crea una tabla de contingencia en las dos variables: pais y sexo
```
